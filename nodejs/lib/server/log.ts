...
import db from './db'
import { scrub } from '@/lib/log/scrub'
 ...
 * never break a chat request, so everything is wrapped and swallowed. `ctx` is
 * deep-scrubbed — an Anthropic key can never reach the table.
 */
  ...
  try {
    ...
    db.prepare(
      `INSERT INTO logs (ts, device_id, request_id, level, event, msg, ctx)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
    ).run(Date.now(), deviceId, requestId ?? null, level, event, msg ? msg.slice(0, 8000) : null, ctxJson)
