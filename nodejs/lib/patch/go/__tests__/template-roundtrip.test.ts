...
test('bass template round-trips the real GO bass patch byte-for-byte', { skip: !fs.existsSync(BASS_FIXTURE) && 'bass fixture absent' }, () => {
  ...
  assert.deepEqual(Object.keys(emitted), Object.keys(golden), 'same block keys, same order')
  for (const k of Object.keys(golden)) {
    assert.deepEqual(emitted[k], golden[k], `bass block ${k} bytes equal`)
  }
})
